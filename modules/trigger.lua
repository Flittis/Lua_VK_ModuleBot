local config = ...
local stickerTriggerWord, stickerTriggerChats, stickerTriggerStickers = config.stickerTriggerWord or '', config.stickerTriggerChats or {}, config.stickerTriggerStickers or {1}
local timeout = {}
...
function obj.func(msg)    -- creating callback function
    ...
    if(timeout[msg.peer_id] == nil and msg.out == false and msg.body:lower():find(stickerTriggerWord) and (not stickerTriggerChats[1] or isInArray(stickerTriggerChats, msg.chat_id))) then
        ...
        timeout[msg.peer_id] = os.time() + 300
    end
end
