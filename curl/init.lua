...
if h then
	...
	error("Can\"t open cURL header file.")
end
...
function curl_request(url, ua)
	...
	if res == lib.CURLE_OK then
		res = lib.curl_easy_setopt(h, lib.CURLOPT_WRITEFUNCTION, writefunc)
		if res == lib.CURLE_OK then
			res = lib.curl_easy_setopt(h, lib.CURLOPT_WRITEDATA, data)
			if res == lib.CURLE_OK then
				res = lib.curl_easy_setopt(h, lib.CURLOPT_USERAGENT, ua)
				if res == lib.CURLE_OK then
					res = lib.curl_easy_perform(h)
				end
			end
		end
	end
	if res == lib.CURLE_OK then
		...
		return nil, ffi.string(lib.curl_easy_strerror(res))
	end
end
